...
/*   Updated: 2023/01/11 13:11:57 by jrinna           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	Client::initiate_cgi_env( std::string const & fullPath ) {
	...
	envm[envm.size()] = std::string("PATH_TRANSLATED=") + _request.getPath();
	envm[envm.size()] = std::string("QUERY_STRING=") + _request.getQueries();
	//envm[envm.size()] = std::string("REMOTE_ADDR=");
	//envm[envm.size()] = std::string("REMOTE_HOST=");
	//envm[envm.size()] = std::string("REMOTE_IDENT");
	//envm[envm.size()] = std::string("REMOTE_USER");
	envm[envm.size()] = std::string("REQUEST_METHOD=") + _request.getMethod();
	envm[envm.size()] = std::string("SCRIPT_NAME=") + _request.getPath();
	envm[envm.size()] = std::string("SERVER_NAME=");
	...
	map_to_env(envm);
	/* for (unsigned long i = 0; _env_malloc[i]; i++)
	{
		cerr << "env : " << i << " : -" << _env_malloc[i] << endl;
	} */
...
void	Client::map_to_env( std::map<int, std::string> envm ) {
	...
	//cerr << "map to env" << endl;
	char	**env = new char*[envm.size() + 1];
	//std::cout << "NEW env : " << env << std::endl;
	env[envm.size()] = NULL;
	//env[0] = NULL;
